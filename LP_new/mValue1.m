function [M_corr ] = mValue1(test_audio,lambda,PB_watermark,PB,cover_audio,seg_len,pa_len,fs,watermark)
%LP优化是计算目标函数值时调用
len = length(cover_audio);
seg_number=floor(len/seg_len);
%[ ~,PB,~ ] = partition( cover_audio,seg_len,seg_number,pa_len,len );
%------------------------压缩攻击--------------------------%
